...
dojo.declare("loc.Monster", loc.Sprite, {
    ...
    dropItem: function monster_dropItem() {
        var rndSeed = Math.random() * 100;
        if (rndSeed < 50) {
            // nothing to drop
        } else if (rndSeed < 65) {
            // drop a heart
            game.insertItem(new loc.Heart({'pos':this.pos}));
        } else if (rndSeed < 80) {
            // drop 1 rupee
            game.insertItem(new loc.Rupee({'pos':this.pos}));
        } else if (rndSeed < 90) {
            // drop 5 rupees
            game.insertItem(new loc.Rupee({'pos':this.pos,'amount':5}));
        } else if (rndSeed < 97) {
            // drop a fairy
            soundManager.play("special");
            game.insertItem(new loc.Fairy({'pos':this.pos}));
        } else {
            // drop a clock
            // TBD
        }
    ...
    getHit: function monster_getHit(damage) {
        if ((this._state == 0 || this._state > 2) && !this._hurtTimer) {
            this.HP -= damage;
            //this._hurtTimer = 10;
            if (this.HP <= 0) {
                this.die();
